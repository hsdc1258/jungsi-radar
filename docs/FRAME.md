...
- **상태 뱃지 둘은 예외로 길다**: `보류`(등급만 입력해 판정 구간이 두 띠에 걸침)와 `기준 불일치`(컷의 통계 정의가 달라 뺄 수 없음). 판정 띠와 같은 자리에 놓고 tone은 neutral. 이 두 상태에서는 차이 숫자를 `—`로 두고, 이유와 필요한 자료는 부제 한 줄과 정보 탭의 '비교 기준' 표가 말한다.
